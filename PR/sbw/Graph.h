...
public:
    ...
    SIZE_TYPE* d_activeNodes;
    EDGE_POINTER_TYPE* d_activeNodesPointer;
    SIZE_TYPE* d_outDegree;
    SIZE_TYPE* d_degree;
...
void Graph<EdgeType>::readDataFromFile(const string& fileName) {
    ...
    cout << "readDataFromFile " << duration << " ms" << "\n";
    // mem
    // h2d+=vertexArrSize*sizeof(uint)*2;
    // h2d+=vertexArrSize*sizeof(ull)*;
    //for (int i = 0; i < 1024; i++)
    //    cout << edgeArray[i] << " " << nodePointers[i] << "\n";
    //exit(0);
