...
void GraphMeta<EdgeType>::initGraphDevice() {
    ...
    if(algType == PR) {
        ...
        valuePrD = malloc_device<double>(vertexArrSize,streamStatic);
        sumD = malloc_device<double>(vertexArrSize,streamStatic);

        streamStatic.memcpy(outDegreeD, outDegree, vertexArrSize * sizeof(SIZE_TYPE));
        streamStatic.memcpy(valuePrD, valuePr, vertexArrSize * sizeof(double));
        streamStatic.memset(sumD, 0, vertexArrSize * sizeof(double));
    } else {
...
void GraphMeta<EdgeType>::getMaxPartitionSize() {
    ...
    size_t G1024=1024;
    size_t availMemory = G1024*G1024*G1024*10;
    long reduceMem = (paramSize-2) * sizeof(SIZE_TYPE) * (long) vertexArrSize;

    reduceMem += sizeof(double) * 2 * (long)vertexArrSize;
    cout << " testNumNodes " << vertexArrSize << " edgeArrSize " << edgeArrSize << " ParamsSize " << paramSize
         << "\n";
    ...
    partOverloadSize = total_gpu_size - max_partition_size; // 显存装不下的overload
    ...
    for (int threadIndex = 0; threadIndex < threadNum; threadIndex++) {
        ...
        runThreads[threadIndex] = thread([&, threadIndex] {
            ...
            for (unsigned int i = left; i < right; i++) {
                ...
                //     cout << "activeOverloadNodePointers[" << i << "] is " << activeOverloadNodePointers[i] << "\n";
                //     break;
                // }
                for (unsigned int j = 0; j < thisDegree; j++) {
                    overloadEdgeList[fromHere + j] = edgeArray[fromThere + j];
                    //cout << fromHere + j << " : " << overloadEdgeList[fromHere + j] << "\n";
                }
...
void GraphMeta<EdgeType>::caculatePartInfoForEdgeList(SIZE_TYPE overloadNodeNum, EDGE_POINTER_TYPE overloadEdgeNum) {
    ...
    if (partOverloadSize < overloadEdgeNum) {
        ...
        while ((activeOverloadNodePointers[right] + degree[overloadNodeList[right]] -
            ...
            //cout << "activeOverloadNodePointers[right] + degree[overloadNodeList[right]] "<< activeOverloadNodePointers[right] + degree[overloadNodeList[right]] <<" activeOverloadNodePointers[left] " << activeOverloadNodePointers[left] << "\n";
            ...
            while (start <= end) {
                ...
                if (headDistance <= partOverloadSize && tailDistance > partOverloadSize) {
                    ...
                    //cout << "activeOverloadNodePointers[mid] "<< activeOverloadNodePointers[mid] <<" activeOverloadNodePointers[left] " << activeOverloadNodePointers[left] << "\n";
